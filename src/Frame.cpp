...
void FrameShowHelpScreen(int sx, int sy) // sx, sy - sizes of current window (screen)
{
    ...
    const char * HelpStrings[] = {
        ...
        " Shift+F6 - Reserved for debugger",
        " F7     - Normal speed. Shift+F7 - Speed up. Ctrl+F7 - Speed down.",
...
void  FrameDispatchMessage(SDL_Event * e) // process given SDL event
{
    ...
    // check caps lock correctly
    int temp = SDL_GetModState();
    temp = temp & KMOD_CAPS;
    if(temp == KMOD_CAPS && !KeybGetCapsStatus()) KeybCapsLockOn();
    if(temp != KMOD_CAPS && KeybGetCapsStatus()) KeybCapsLockOff();
    ...
    case SDL_KEYDOWN:
        ...
        if(mysym < 128 && mysym != SDLK_PAUSE) { // it should be ASCII code?
            ...
            {
                g_dwSpeed = SPEED_NORMAL;// dw is unsigned value!
                printf("Now speed=%d\n", (int)g_dwSpeed);
                SetCurrentCLK6502();
            }
...
bool PSP_SaveStateSelectImage(bool saveit)
{
    ...
    while(isdir)
    {
        if(!ChooseAnImage(g_ScreenWidth, g_ScreenHeight, fullpath, saveit, &filename, &isdir, &findex)) {
            DrawFrameWindow();
...
void ProcessButtonClick (int button, int mod) {
    ...
    case BTN_HELP:  // F1 - ill get some help on the screen?
    ...
    case BTN_FULLSCR: // F6 - Fullscreen on/off Shift+F6 - Debugger
        if (mod & KMOD_SHIFT) {
            /*    if (g_nAppMode == MODE_LOGO)
              {
                ResetMachineState();
    ...
    case BTN_SPEED: // F7 - speed (shift increases, Ctrl decreases)
        if(mod & KMOD_CTRL) {
            if(g_dwSpeed > SPEED_MIN) g_dwSpeed = g_dwSpeed - 1;// dw is unsigned value!
            //if(g_dwSpeed <= SPEED_MIN) g_dwSpeed = SPEED_MIN; // no Minimum tresspassing!
            printf("Now speed=%d\n", (int)g_dwSpeed);
    ...
    case BTN_SETUP: // F8 - setup is in conf file - linapple.conf.
    ...
    case BTN_SAVEST:  // F11 - Save state (F11)
        ...
        break;
    case BTN_LOADST:  // F12 - Load state (F12) or Hot Reset (Ctrl+F12)
        if(mod & KMOD_CTRL) {
            // Ctrl+Reset
            if (!IS_APPLE2)
                MemResetPaging();
