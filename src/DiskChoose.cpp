...
bool ChooseAnImage(int sx,int sy, char *incoming_dir, int slot, char **filename, bool *isdir, int *index_file)
{
    ...
    */
    ...
    while(true)
    {
        ...
        else if (slot == 7) font_print_centered(sx/2,20*facy,"Choose image for Hard Disk", screen, 1*facx, 1*facy);
        else if (slot == 5) font_print_centered(sx/2,20*facy,"Choose image for floppy 800KB drive", screen, 1*facx, 1*facy);
        else if (slot == 1) font_print_centered(sx/2,20*facy,"Select file name for saving snapshot", screen, 1*facx, 1*facy);
        else if (slot == 0) font_print_centered(sx/2,20*facy,"Select snapshot file name for loading", screen, 1*facx, 1*facy);
        ...
        while(files.Iterate(tmp)) {
            ...
            {   // FILES_IN_SCREEN items on screen
                ...
                ch = 0;
                if(strlen(tmp) > 46) {
                    ch = tmp[46];    //cut-off too long string
                    tmp[46] = 0;
                }
